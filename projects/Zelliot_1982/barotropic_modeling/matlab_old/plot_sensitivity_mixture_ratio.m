%% Sensitivity of barotropic model to composition
% Clear the workspace
clear all
close all
clc
...
exp_data = readtable("../experimental_data/experimental_cases.xlsx");
...
for k = 1:numel(prop_names)
    exportgraphics(figs.(prop_names{k}), fullfile(output_dir, ['sensitivity_mixture_ratio_', prop_names{k} ,'.png']), Resolution=750)
end

%%
for k = 1:numel(prop_names)
    export_fig(figs.(prop_names{k}), fullfile(output_dir, ['sensitivity_mixture_ratio_', prop_names{k} ,'.svg']), '-svg');
end

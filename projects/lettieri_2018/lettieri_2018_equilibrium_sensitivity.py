...
# Read Case Summary
DATAFILE = "./nakagawa_2009_data.xlsx"  # Case summary file
...
fluid.plot_phase_diagram(prop_x, prop_y, axes=ax1, plot_quality_isolines=True, plot_spinodal_line=True)
ax1.set_ylim([fluid.triple_point_liquid.p, 1.5*fluid.critical_point.p])

# Efficiency sensitivity analysis
q_onset_array = [0.00, 0.05, 0.10]
...
for i, (index, row) in enumerate(case_data.iterrows()):
    ...
    p_in = row["p0_in"]
    p_out = fluid.triple_point_vapor.p

...
def calculate_barotropic_process(calculation_type="polytropic", **kwargs):
    """
    """
    # Rename parameters
    fluid_name = kwargs.get("fluid_name")
    T0_in = kwargs.get("T_in")
    p0_in = kwargs.get("p_in")
    p_out = kwargs.get("p_out")
    efficiency = kwargs.get("efficiency")
    n_points = kwargs.get("number_of_points")
    ...
    if calculation_type == "adiabatic":
        ...
        states, _ = calculate_polytropic_process(
            fluid_name=fluid_name,
            T_in=T0_in,
            p_in=p0_in,
            p_out=p_out,
...
def calculate_adiabatic_process(
    ...
    if number_of_points is not None:
        T_array = np.linspace(state_in.T, state_out.T, number_of_points)
        s_array = np.linspace(state_in.s, state_out.s, number_of_points)
        states = []
        for T, s in zip(T_array, s_array):
            states.append(fluid.set_state(props.SmassT_INPUTS, s, T))
    else:

...
p = 50e5
spndl_liq = bpy.compute_spinodal_point_general(
    "p", p, fluid, branch="liquid", supersaturation=True
)
spndl_vap = bpy.compute_spinodal_point_general(
    "p", p, fluid, branch="vapor", supersaturation=True
)
...
state = fluid.get_state(bpy.PSmass_INPUTS, p, s1)
states_liquid = []
...
states_liquid = bpy.states_to_dict(states_liquid)
...
states_vapor = bpy.states_to_dict(states_vapor)
...
states_equilibrium = []
for i, s in enumerate(s_array):
    state = fluid.get_state(bpy.PSmass_INPUTS, p, s)
...
bpy.savefig_in_formats(fig, os.path.join(fig_dir, f"supersaturation_along_isobar_Ts_{fluid.name}"))
...
bpy.savefig_in_formats(fig, os.path.join(fig_dir, f"supersaturation_along_isobar_ps_{fluid.name}"))
